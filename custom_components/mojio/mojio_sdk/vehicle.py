...
class Vehicle(AbstractMojio):
    ...
    def __init__(self, json_data):
        ...
        # Some tenants (e.g. Audi) also return a separate 'MojioId' that differs
        # from 'Id' - it identifies the Mojio device rather than the vehicle.
        # Trips are keyed against 'Id' (verified against the live Audi tenant),
        # so use mojio_id for trip matching.
        self.alt_mojio_id = json_data.get('MojioId')
        ...
        # Speed and RPM
        speed_dict = json_data.get('Speed') or {}
        rpm_dict = json_data.get('RPM') or {}
        current_speed = speed_dict.get('Value', None)
        self.current_rpm_unit = rpm_dict.get('Unit', '')
        self.current_speed_unit = speed_dict.get('Unit', '')
        if current_speed is not None and self.ignition_state:
            self.current_rpm = rpm_dict.get('Value', None)
        ...
        # Parked and motion
        self.parked = (json_data.get('ParkedState') or {}).get('Value', 'unknown')
        self.status = (json_data.get('VehicleStatus') or {}).get('Value', 'unknown')
        # Heading and Turn
